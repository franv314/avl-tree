/*
 * Task: https://judge.yosupo.jp/problem/predecessor_problem
 * Tests: Everything
 */
...
int main() {
    ...
    AVLTree<int64_t, monostate> map;
    int N, Q; cin >> N >> Q;

    string S; cin >> S;
    for (int i = 0; i < N; i++) {
        if (S[i] == '1') {
            map.insert(i, monostate());
        }
    }
    ...
    while (Q--) {
        ...
        switch (t) {
        case 0: map.insert(k, monostate()); break;
        case 1: map.erase(k); break;
        case 2: cout << map.count(k) << '\n'; break;
        case 3: cout << map.succ(k).value_or(make_pair(-1, monostate())).first << '\n'; break;
        case 4: cout << map.prec(k).value_or(make_pair(-1, monostate())).first << '\n'; break;
        }
    }
}